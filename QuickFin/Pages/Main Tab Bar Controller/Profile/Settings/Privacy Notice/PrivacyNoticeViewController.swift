...
class PrivacyNoticeViewController: BaseViewController {
...
extension PrivacyNoticeViewController {
    ...
    func initUI() {
        let privacyNoticeTextView: UITextView = {
            let v = UITextView()
            v.isEditable = false
            v.text = privacyNotice
            v.textColor = Colors.DynamicTextColor
            v.font = .systemFont(ofSize: FontSizes.text)
            v.scrollsToTop = true
            return v
        }()
        
        view.addSubview(privacyNoticeTextView)
        privacyNoticeTextView.snp.makeConstraints { (this) in
